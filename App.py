...
def agrupar_por_campanha():
    # Cria um dicionário para somar valores por campanha
    resumo = {}
    for lead in st.session_state.leads_data:
        camp = lead['Campanha']
        val = lead['Valor'].replace('R$', '').replace('.', '').replace(',', '.').strip()
...
# ==========================================
st.set_page_config(page_title="EcoData Performance", page_icon="📊", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    <style>
        ...
        [data-testid="stHorizontalBlock"] button { background-color: transparent !important; border: none !important; color: #94a3b8 !important; font-size: 13px !important; font-weight: 500 !important; }
...
with nav_col2:
    ...
    if b1.button("📊 Visão", use_container_width=True): st.session_state.pagina_atual = '📊 Visão Geral'
    if b2.button("🎯 Camp", use_container_width=True): st.session_state.pagina_atual = '🎯 Campanhas'
    if b3.button("👥 Leads", use_container_width=True): st.session_state.pagina_atual = '👥 Leads'
    if b4.button("🔍 Logs", use_container_width=True): st.session_state.pagina_atual = '🔍 Rastreamento e Logs'
    if b5.button("📄 Relat", use_container_width=True): st.session_state.pagina_atual = '📄 Relatórios'
    if b6.button("⚙️ Config", use_container_width=True): st.session_state.pagina_atual = '⚙️ Configurações'
...
if st.session_state.pagina_atual == '📊 Visão Geral':
    ...
    with m2: st.markdown(f"<div class='metric-box'><div class='metric-title'>[FATURAMENTO]</div><div class='metric-value'>{calcular_faturamento()}</div></div>", unsafe_allow_html=True)
    with m3: st.markdown(f"<div class='metric-box'><div class='metric-title'>[ROAS]</div><div class='metric-value'>3.13x</div></div>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown('<div class="block-header">DISTRIBUIÇÃO DE RECEITA POR CAMPANHA</div>', unsafe_allow_html=True)
        resumo_campanhas = agrupar_por_campanha()
        tabela_atrib = '<table class="custom-table"><thead><tr><th>Campanha</th><th>Receita Gerada</th></tr></thead><tbody>'
        for camp, val in resumo_campanhas.items():
            valor_formatado = f"R$ {val:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            tabela_atrib += f"<tr><td>{camp}</td><td style='color: #38bdf8; font-weight: bold;'>{valor_formatado}</td></tr>"
        tabela_atrib += "</tbody></table>"
        st.markdown(tabela_atrib, unsafe_allow_html=True)

elif st.session_state.pagina_atual == '👥 Leads':
    with st.container(border=True):
        st.markdown('<div class="block-header">👥 GESTÃO E CAPTURA</div>', unsafe_allow_html=True)
        with st.expander("➕ Simular Nova Conversão", expanded=False):
            with st.form("form_simulador"):
                n = st.text_input("Nome", "Cliente Novo")
                v = st.text_input("Valor", "197,00")
                camp = st.selectbox("Campanha", ["Campanha Scale Q3", "Campanha Remarketing", "Campanha Lançamento"])
                if st.form_submit_button("🚀 Registrar Venda"):
                    st.session_state.leads_data.insert(0, {"Horário": datetime.now().strftime("%H:%M:%S"), "Nome": n, "E-mail": "cliente@teste.com", "Telefone": "00000", "Campanha": camp, "Valor": f"R$ {v}", "Status": "🟢 Convertido"})
                    st.rerun()

        tabela_html = '<table class="custom-table"><thead><tr><th>Horário</th><th>Campanha</th><th>Valor</th></tr></thead><tbody>'
        for lead in st.session_state.leads_data:
            tabela_html += f"<tr><td>{lead['Horário']}</td><td>{lead['Campanha']}</td><td style='color: #22c55e;'>{lead['Valor']}</td></tr>"
